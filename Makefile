# This Makefile is for the TAP::Stream extension to perl.
#
...
#     ABSTRACT => q[Combine multiple TAP streams with subtests]
#     AUTHOR => [q[Curtis "Ovid" Poe <ovid@cpan.org>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker=>q[6.30] }
#     DISTNAME => q[TAP-Stream]
#     EXE_FILES => []
#     LICENSE => q[perl]
#     NAME => q[TAP::Stream]
#     PREREQ_PM => { Moose::Role=>q[0], namespace::autoclean=>q[0], Test::Most=>q[0], Moose=>q[0] }
#     VERSION => q[0.44]
#     test => { TESTS=>q[t/*.t] }
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/TAP/Stream.pm \
	lib/TAP/Stream/Role/ToString.pm \
	lib/TAP/Stream/Text.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/TAP
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/TAP
...
TO_INST_PM = lib/TAP/Stream.pm \
	lib/TAP/Stream/Role/ToString.pm \
	lib/TAP/Stream/Text.pm

PM_TO_BLIB = lib/TAP/Stream.pm \
	blib/lib/TAP/Stream.pm \
	lib/TAP/Stream/Role/ToString.pm \
	blib/lib/TAP/Stream/Role/ToString.pm \
	lib/TAP/Stream/Text.pm \
	blib/lib/TAP/Stream/Text.pm
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/TAP/Stream.pm $(INST_MAN3DIR)/TAP::Stream.$(MAN3EXT) \
	  lib/TAP/Stream/Role/ToString.pm $(INST_MAN3DIR)/TAP::Stream::Role::ToString.$(MAN3EXT) \
	  lib/TAP/Stream/Text.pm $(INST_MAN3DIR)/TAP::Stream::Text.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:               TAP-Stream' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:            0.44' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:           Combine multiple TAP streams with subtests' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Curtis "Ovid" Poe <ovid@cpan.org>' >> META_new.yml
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0.44">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Combine multiple TAP streams with subtests</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Curtis "Ovid" Poe &lt;ovid@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Moose::" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Moose::Role" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::Most" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/TAP/Stream.pm blib/lib/TAP/Stream.pm \
	  lib/TAP/Stream/Role/ToString.pm blib/lib/TAP/Stream/Role/ToString.pm \
	  lib/TAP/Stream/Text.pm blib/lib/TAP/Stream/Text.pm 
